...
function createGit(dir, context) {
  ...
  function git(args, options) {
    return exec('/usr/bin/git', args, options)
  }
  function child(subdir) {
    return createGit(path.join(dir, subdir), context)
  }
  exec.git = git
  exec.child = child
  exec.exec = exec
...
async function getWorktree(slug, context, token) {
  ...
  try {
    ...
    await worktree.git(['config', '--local', 'http.postBuffer', '524288000'])
  }
